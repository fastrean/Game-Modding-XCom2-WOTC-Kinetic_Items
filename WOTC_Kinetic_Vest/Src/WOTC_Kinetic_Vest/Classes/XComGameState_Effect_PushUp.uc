class XComGameState_Effect_PushUp extends XComGameState_Effect;// config(HybirdBulletsAmmoData);
...
function EventListenerReturn KineticPlatingListener(Object EventData, Object EventSource, XComGameState GameState, Name EventID, Object CallbackData)
{
	...
	local X2Effect_PushUpVestPersonalKineticShield PlatingEffect;
	...
	if (AbilityContext != none && AbilityContext.InterruptionStatus != eInterruptionStatus_Interrupt)
	{
		...
		{
			PlatingEffect = X2Effect_PushUpVestPersonalKineticShield(GetX2Effect());
			...
			if (UnitState != none)
			{
				...
				UnitState.ModifyCurrentStat(eStat_ShieldHP, PlatingEffect.ShieldPerCharge);
